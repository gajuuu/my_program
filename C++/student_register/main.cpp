...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            // TODO: Add functionality here
            }else if(student_numbers.find(parts.at(1))
                     == student_numbers.end()){
                ...
                getline(std::cin, newPhoneNum);
                std::cout << std::endl;

                if(!is_valid_phone_number(newPhoneNum)){
                    continue;

                }else{
                    Student* givenStudentLocation = student_numbers.at(parts.at(1));
                    givenStudentLocation->phone_number = newPhoneNum;
                    std::ofstream writingFile;
                    writingFile.open(file_name);
                    for(auto& pairData: user_ids){
                        writingFile << pairData.second->student_number << ";" <<
                                       pairData.first << ";" <<
                                       pairData.second->name << ";" <<
                                       pairData.second->phone_number << ";" <<
                                       pairData.second->email << ";" <<
                                       pairData.second->skype << std::endl;
                    }
                    writingFile.close();
                }
            }
